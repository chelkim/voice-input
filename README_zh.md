...
语音输入是一款基于全局热键的语音听写工具，专为 Linux 系统设计。只需按 `Alt+Q` 或 `Alt+W` 即可开始录音，系统会自动将语音转写为文字并粘贴到当前焦点输入框。支持中文、英文、日文、韩文、粤语等多种语言，其中中文识别率最高。UI 语言会根据你的使用习惯自动适应。完美支持 X11 和 Wayland 桌面会话，开机自动启动，无需网络，离线运行。

## 功能特性

- **双快捷键** - `Alt+Q`（自动回车）或 `Alt+W`（手动回车）
...
1. 确保守护进程在运行（开机后自动启动）
2. 鼠标点击目标输入框
3. 按 `Alt+Q`（自动回车）或 `Alt+W`（手动回车）开始录音
4. 说话
5. 再按 `Alt+Q` 或 `Alt+W` 停止录音
6. 文字自动填入输入框
   - 使用 `Alt+Q`：粘贴后自动按回车
   - 使用 `Alt+W`：粘贴后不按回车（手动）
7. 录音过程中按 `ESC` 可取消
...
# Alt+Q: 自动回车模式（粘贴后自动按回车）
# Alt+W: 手动回车模式（粘贴后不按回车）
key = Ctrl+Q
key2 = Ctrl+W
